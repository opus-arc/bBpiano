...
显然，**这在数学分析上构成了极其标准的傅里叶正弦级数展开（Fourier Sine Series）。** 为了显式求解出每一阶谐波分量的振幅系数 $A_m$，我们两边同时乘以 $\sin\left(\frac{k\pi x}{L}\right)$ 并利用经典**三角函数族的正交性（基本上所有的数学分析的课本都会在讲傅立叶级数这一章的最初提到）**，在区间 $[0, L]$ 上进行定积分。当 $m \neq k$ 时，基函数的内积积分严格为 0；唯有当 $m = k$ 时内积非零，由此我们可以精确导出 $A_m$ 的积分表达式：

```math
A_m = \frac{2}{L} \int_{0}^{L} f(x) \sin\left(\frac{m\pi x}{L}\right) dx
```

这个源自数学分析II的积分公式，直接让我们把纯弦模型的致命漏洞扒了个精光：**如果我们不引入独立的动态激发机制，仅仅是在代码初始化时强行喂给弦一个固定几何形状的初始扰动 $f(x)$，那么所有谐波分量 $A_m$ 之间的相对比例关系将被这个空间积分永远锁死。**
...
由于琴槌毛毡具有复杂的非线性弹性特征，当琴槌以初始击弦速度撞击琴弦时，它施加给弦的压力完全取决于琴槌自身运动位移 $y_h(t)$ 与琴弦在接触点处的实时动态位移 $y(x_c, t)$ 之间的相对压缩量 $\Delta x(t)$。这种强烈的**双向动态耦合（Bidirectional Coupling）性**，使得力 $F(x_c, t)$ 本身成为了系统当前状态的非线性函数：
...
因此，为了打破纯线性齐次系统的死板状态，让系统能够根据唯一的物理输入参数——**击弦速度（Impact Velocity）**自动演化出形态各异、随力度非线性改变的强迫项力信号 $F(x_c, t)$，我们必须将目光从琴弦中抽离出来，去从零构建一个融合了**运动差分骨架与瞬时非线性毛毡映射**的动力学核心——**琴槌模型**。

在我们开始用数字导波管（Digital Waveguide, DWG）具体实现琴槌激发之前，我们需要跳出纯数学的圈子，站在更宏观的系统工程视角来审视 Bank 钢琴整体建模的哲学。

在构建钢琴物理建模的整体框架时，我们经常需要在“绝对的物理真实”与“计算效率”之间做出残酷的权衡。为了让整个算法能够在现代计算机中实时运行（Real-time Execution），任何一个钢琴物理建模的研究者都不可避免地需要在一些次要效应（Secondary Effects）上采用“非物理（Nonphysical）”或纯信号驱动的简化。例如，我们在前面讨论的音板（Soundboard）三维振动，其真实的偏微分方程计算量巨大，因此我们通常会用一个无物理实体解释、但听觉特征高度逼近的线性时不变（LTI）多输入单输出滤波器来“敷衍”过去。
...
然而，当我们试图把这个纯正的连续时间力学方程，塞进离散时间的数字导波管（DWG）时，一个重大的算法危机随即爆发。
