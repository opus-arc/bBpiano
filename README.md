...
在构建钢琴物理建模的整体框架时，我们经常需要在“绝对的物理真实”与“计算效率”之间做出残酷的权衡。为了让整个算法能够在现代计算机中实时运行（Real-time Execution），任何一个钢琴物理建模的研究者都不可避免地需要在一些次要效应（Secondary Effects）上采用“非物理（Nonphysical）”或纯信号驱动的简化。
