...
struct PedalCurveView: View {
    @StateObject var model = CurveModel()
    @Binding var isDarkMode: Bool
    @Binding var currentTab: CurveTab
    @State private var isCalibrating: Bool = false

    init(isDarkMode: Binding<Bool>, currentTab: Binding<CurveTab>) {
        _isDarkMode = isDarkMode
        _currentTab = currentTab
        // 前后各有一段死区横线
        let initialPoints = [
            ControlPoint(x: 0, y: 0),
            ControlPoint(x: 0.2, y: 0),
            ControlPoint(x: 0.8, y: 1),
        ...
        ZStack {
            ...
            VStack {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    CurveHeaderView(currentTab: $currentTab, isDarkMode: $isDarkMode, theme: theme)
                            ...
                            GeometryReader { geometry in
                                ...
                                ZStack {
                                    ...
                                    Path { path in
                                        ...
                                        path.addLine(to: CGPoint(x: size.width * 0.8, y: 0))
                                        path.addLine(to: CGPoint(x: size.width, y: 0))
                                    }.stroke(isDarkMode ? Color.white.opacity(0.35) : Color.black.opacity(0.2), style: StrokeStyle(lineWidth: 1.2, dash: [4, 4]))
                                    ...
                                    ForEach(model.points) { point in if let index = model.points.firstIndex(where: { $0.id == point.id }) { ControlPointView(point: point, size: size, handleColor: theme.handleColor, strokeColor: theme.handleStroke).gesture(DragGesture(minimumDistance: 0).onChanged { handleDrag(at: index, value: $0, size: size) }.onEnded { _ in model.updateLUT() }) } }
                    ...
                    HStack(spacing: 15) {
                        Button(action: { withAnimation(.easeInOut(duration: 0.2)) { isCalibrating.toggle() } }) { Text("Calibration") }.buttonStyle(ControlButtonStyle(isToggled: isCalibrating, theme: theme, isDarkMode: isDarkMode))
                        Button(action: { withAnimation(.interpolatingSpring(stiffness: 300, damping: 15)) {
                            // 同步修改 Reset 行为
                            model.points = [ControlPoint(x: 0, y: 0), ControlPoint(x: 0.2, y: 0), ControlPoint(x: 0.8, y: 1), ControlPoint(x: 1, y: 1)]
                            model.updateLUT()
                        } }) { Text("Reset") }.buttonStyle(ControlButtonStyle(isToggled: false, theme: theme, isDarkMode: isDarkMode))
                        Text("G").font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(isDarkMode ? Color.white.opacity(0.7) : Color.black.opacity(0.7)).frame(width: 22, height: 22).background(Circle().stroke(isDarkMode ? Color.white.opacity(0.5) : Color.black.opacity(0.2), lineWidth: 1)).padding(.leading, 80)
