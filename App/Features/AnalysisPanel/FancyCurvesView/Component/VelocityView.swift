...
struct VelocityView: View {
    ...
    init(currentTab: Binding<CurveTab>) {
        ...
        _model = StateObject(
            wrappedValue: CurveModel(points: CurveModel.velocityCurvePoints)
        )
    }
    ...
    // 将 MIDI velocity (0...127)
    // 按照当前曲线控制点映射到新的 velocity 值
    func mappedVelocity(from midiVelocity: Int) -> Double {
