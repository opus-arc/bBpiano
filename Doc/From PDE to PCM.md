...
我采用的变量体系是 **Bank / Smith 的 velocity-wave digital waveguide**：弦状态用左右行波 v^+,v^- 表示，物理速度为两者之和；锤子用 lumped mass + nonlinear spring；锤子子系统跑 2f_s，弦跑 f_s。

对第 r 个击弦点 x_r，设其空间窗函数为 g_r(x,x_r)，则总外力密度写成

$$f(x,t)=\sum_{r=1}^{R} F_r(t)\, g_r(x,x_r)$$
...
我采用的变量体系是 Bank / Smith 的 velocity-wave digital waveguide：弦状态用左右行波 v^+,v^- 表示，物理速度为两者之和；锤子用 lumped mass + nonlinear spring；锤子子系统跑 2f_s，弦跑 f_s。这正是 Bank 方案的核心结构   。
...
这就是把“击弦点不止一个”写成总外力密度的方式；Chaigne 的 PDE 写法就是这种 f(x,x_0,t)=F_h(t)g(x,x_0) 的形式 。
...
k_r \, \Delta y_r(t)^{p_r}, & \Delta y_r(t)>0\\[4pt]
...
\Delta y_r(t)=y_{h,r}(t)-y_{s,r}(t)



F_r(t)=-m_{h,r}\,\ddot y_{h,r}(t)



这三式就是 Bank/综述中的标准 hammer–string 接触律 。
...
F_r(t)=k_r\,[1-h_r(t)] * [\Delta y_r(t)^{p_r}]



其中 h_r(t) 为一阶 relaxation kernel；综述给出的形式正是这样 。
...
v_{s,r}(nT_s)=v^+(n,m_r)+v^-(n,m_r)



这就是弦上该点的局部物理速度，来自左右行波求和 。
...
3. Bank 的关键改造：锤子不直接读“同点位移”，而是单独维护 excitation-point state
...
y_{s,r}(t)=\int v_{s,r}(t)\,dt



Bank 的关键不是把这个量直接从 naïve waveguide 同点读出，而是在 hammer 子系统里单独积分该点速度，从而规避 discontinuity problem；论文对此有明说 。



于是 hammer compression 仍然是
...
但其中 y_{s,r} 来自 hammer 子系统自己的局部状态更新，而不是简单地“激励点读值再直接相加”。
...
4. 多速率：把弦的 f_s 输入提升到锤子的 2f_s



对第 r 个击弦点，弦在正常采样率 f_s 的速度输入为



v_{in,r}(nT_s)=v^+(n,m_r)+v^-(n,m_r)
...
v_{in,r}\!\left(nT_s+\frac{T_s}{2}\right)=

\frac{v^+(n,m_r)+v^+(n,m_r-1)}{2}
...
这两个值喂给 hammer 子系统，使 hammer 在 2f_s 下跑两步。
...
5. 在 2f_s 下积分 hammer 与 excitation-point displacement
...
\Delta t_h=\frac{T_s}{2}



对第 r 个 hammer，在双速率索引 \nu 下：
...
-\frac{\Delta t_h^2}{m_{h,r}}\,F_r^{(\nu)}



5.3 compression 与 force
...
若含 hysteresis，就把 \Delta y_r^{(\nu)} 先过一阶 IIR 再进非线性；Bank 也明确指出可以这样级联实现 。
...
6. 从 hammer force 回到弦：Bank 的下采样



hammer 在一个字符串采样周期内会输出两个 force 样本。Bank 的回写公式是 
...
F_{r}(nT_s)+F_{r}\!\left(nT_s+\frac{T_s}{2}\right)
...
若采用速度波变量，则第 r 个击弦点对波导的局部注入写成



\delta_r(n)=\frac{F_{in,r}(n)}{2Z_{0,r}}



其中 Z_{0,r} 为该弦特性阻抗。对理想弦，Bank thesis 给出
...
v^+(n,m_r)\leftarrow v^+(n,m_r)+\delta_r(n)
...
若存在多个击弦点，则在同一采样时刻把所有击弦点注入线性叠加：



v^+(n,m)\leftarrow v^+(n,m)+\sum_{r:\,m_r=m}\delta_r(n)
...
8. Bank 的 discontinuity 修正



Bank 指出：对 nonlinear interaction，不能天真地在同一点把激励一次性同时加满，否则激励点处位移会出错。推荐修正为



方式 A：半量分两步

\text{先在 }n\text{ 时刻加 } \frac{1}{2}\delta_r(n)\text{ 到两条延迟线}

\text{再在 }n+1\text{ 时刻补上剩余的 } \frac{1}{2}\delta_r(n)



方式 B：等效实现

先在时刻 n 只加到上行波 v^+ 的 m_r，

再在时刻 n+1 加到下行波 v^- 的 m_r-1。



更简洁的实际实现，就是采用上一节的 Bank 方案：

不从 naïve 同点位移反馈，而在 hammer 模块内部跟踪 excitation-point displacement。这也是他最终主张的实现法 。
...
对每根弦，传播由双向 delay line 给出。若总回路延迟为 N_r，则理想传播是
...
若把所有分布损耗与色散集中到一个端点，则反射滤波器为



H_{r}(z)=-H_l(z)\,H_d(z)\,H_{fd}(z)
...
​	•	H_d(z)：dispersion allpass

​	•	H_{fd}(z)：fractional-delay fine tuning



这是综述里对 Bank/Smith string block 的标准分解 。
...
v_r^-(n,0)=\mathcal{R}_{\text{nut},r}\{v_r^+(n,0)\}



v_r^+(n,M_r)=\mathcal{R}_{\text{bridge},r}\{v_r^-(n,M_r)\}



其中 \mathcal{R} 对应上述滤波反射算子。若取刚性端点的最简情况，则 Smith 给出的是符号翻转 ：
...
y_r(n,m)=y_r(n-1,m)+T_s\,v_r(n,m)



因此，第 r 个击弦点注入 \delta_r(n) 后，对所有点的影响不是重新求解一遍 PDE，而是通过 delay line 的移位与边界反射自动传播到所有点：
...
v(n,m)=\sum_{r=1}^{R}\Bigl(v_r^+(n,m)+v_r^-(n,m)\Bigr)



若是同一根弦上多个外力点，这就是在同一波导上的多点注入；

若是多根弦，则每根弦各自 propagation，再在桥/音板级耦合或输出级求和。
...
v^+(n,M)=\mathcal{R}_{\text{bridge}}\{v^-(n,M)\}



v^-(n,0)=\mathcal{R}_{\text{nut}}\{v^+(n,0)\}
...
\mathcal{P}_r^+\!\bigl(\delta_r\bigr)(n,m)
...
其中 \mathcal{P}_r^\pm 表示“从第 r 个击弦点注入后，经 delay line 平移、端点反射滤波，到达点 m 的左右行波贡献”。



也就是说：

​	•	局部：先算 F_r \rightarrow \delta_r

​	•	全局：再靠波导的移位和反射，把这一增量送到全弦所有点
...
$$\Delta v[n]=v_h[n]-v_{in,h}[n]-\frac{F[n-1]}{2Z_0}$$
...
$$\Delta y[n]=\Delta y[n-1]+T_s\Delta v[n]$$
...
$$F[n]= \begin{cases} K(\Delta y[n])^p,& \Delta y[n]>0\\ 0,& \Delta y[n]\le 0 \end{cases}$$
...
$$v_h[n+1]=v_h[n]-\frac{T_s}{m_h}F[n]$$
