...
struct PedalView: View {
    var body: some View {
        ...
        return ClickableSceneView(
            ...
            onNodeClick: { node in
                guard let pedalNode = pedalRootNode(from: node) else { return }
                rotatePedalClockwise60DegreesOnXAxis(pedalNode)
            }
        )
...
//        scene.rootNode.addChildNode(rod4)
    ...
    func AutorotatePedalClockwise60DegreesOnXAxis(targetStatus: Bool) {
        ...
        let pedalNames = ["leftPedal", "rightPedal", "thirdPedal", "fourthPedal"]

        for pedalName in pedalNames {
            guard let pedalNode = scene.rootNode.childNode(withName: pedalName, recursively: true) else {
                continue
            }
    ...
    func rotatePedalClockwise60DegreesOnXAxis(
        ...
        let restingXValue = pedalNode.value(forKey: "restingEulerX") as? NSNumber
        let restingX = restingXValue?.floatValue ?? Float(pedalNode.eulerAngles.x)
        let isPressedValue = pedalNode.value(forKey: "isPressed") as? NSNumber
        let isPressed = isPressedValue?.boolValue ?? false

        let pressedX = restingX + (Float.pi / 11)
        let targetX = isPressed ? restingX : pressedX
        ...
        if isPressed {
            ...
            if let indexValue = pedalNode.value(forKey: "pedalIndex") as? NSNumber {
    ...
    func makePedalNode(
        ...
        node.setValue(NSNumber(value: Float(initialEulerAngles.x)), forKey: "restingEulerX")
    ...
    struct ClickableSceneView: NSViewRepresentable {
        ...
        final class Coordinator: NSObject {
            var onNodeClick: (SCNNode) -> Void
            weak var sceneView: SCNView?

            init(onNodeClick: @escaping (SCNNode) -> Void) {
